...
module.exports = function registerInvoicesRoutes(app, deps) {
  const {
    ...
    getMerchantProfileIdByUser, getBrandProfileIdByUser,
    ...
    calculatePointsWithFraction, normalizeRoleType, resolveMerchantProfileIdByKey, autoMatchLineItemToBrand,
...
app.post('/api/invoices/:id/line-items', auth, async (req, res) => {
  ...
  try {
    await client.query('BEGIN');
    const access = await canManageInvoice(client, req.user, invoiceId, null, getMerchantProfileIdByUser);
    if (!access.allowed) {
      await client.query('ROLLBACK');
