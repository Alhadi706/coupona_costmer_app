async function processExpiredRewardClaims(client, id) {
  const rows = (await client.query(
    `SELECT id, owner_id, reward_id, points_cost, points_deducted_at
       FROM reward_claims
  ...
  for (const row of rows) {
    const wasDeducted = Boolean(row.points_deducted_at);
    await client.query(
      "UPDATE reward_claims SET status = $2, updated_at = NOW() WHERE id = $1",
      [row.id, wasDeducted ? 'refunded_as_points' : 'expired']
    );
    if (wasDeducted) {
