...
bool shouldAddClaimTransaction(
  ...
  final claimReference =
      (claim['reference'] ?? 'reward_claim:${claim['id'] ?? ''}').toString();
  return !ledger.any(
    (entry) => (entry['reference'] ?? '').toString() == claimReference,
  );
}
...
class _MyRewardsScreenState extends State<MyRewardsScreen> {
    ...
    try {
      ...
      setState(() {
        ...
        _rewards = List<Map<String, dynamic>>.from(
          (results[1] as Iterable? ?? const []).map(
            (e) => Map<String, dynamic>.from(e as Map),
          ),
        );
        _ledger = List<Map<String, dynamic>>.from(
          (results[2] as Iterable? ?? const []).map(
            (e) => Map<String, dynamic>.from(e as Map),
          ),
        );
        _claims = List<Map<String, dynamic>>.from(
          (results[3] as Iterable? ?? const []).map(
            (e) => Map<String, dynamic>.from(e as Map),
          ),
        );
  ...
  Future<void> _openDynamicVoucherSheet() async {
    ...
    final voucher =
        result['voucher'] as Map<String, dynamic>? ?? const <String, dynamic>{};
  ...
  String legacyRewardCategoryKey(Map<String, dynamic> item) {
    final category =
        (item['category'] ?? item['storeCategory'] ?? item['activity'] ?? '')
            .toString()
            .toLowerCase();
    final title = (item['reward_name'] ?? item['title'] ?? '')
        .toString()
        .toLowerCase();
    final store = (item['storeName'] ?? item['merchant_name'] ?? '')
        .toString()
        .toLowerCase();
  ...
  Widget _buildRewardsSectionTabs() {
    return RewardsSectionTabs(
      selectedTab: _sectionTab,
      onChanged: (value) => setState(() => _sectionTab = value),
    );
  }

  Widget _buildCategoryFilterChips() {
    return RewardsCategoryFilterChips(
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.embedded) {
      return Container(color: const Color(0xFFF8F9FA), child: body);
    }
  ...
  Widget _buildRewardsBody() {
    ...
    final filteredUnlocked = unlockedItems.where((item) {
      if (_selectedMerchantId != null) {
        final srcId =
            (item['source_id'] ?? item['sourceId'] ?? item['merchant_id'] ?? '')
                .toString();
        final name =
            (item['storeName'] ??
                    item['merchant_name'] ??
                    ...
                    '')
                .toString();
        if (srcId.isNotEmpty && srcId == _selectedMerchantId) {
          return true;
        }
        if (_selectedMerchantName != null &&
            name.contains(_selectedMerchantName!)) {
          return true;
        }
        return false;
      }
      if (_selectedCoalitionId != null) {
        final cId = (item['coalition_id'] ?? item['source_id'] ?? '')
            .toString();
        final origin = (item['origin'] ?? '').toString();
        if (cId == _selectedCoalitionId || origin == 'coalition') {
          return true;
        }
        return false;
      }
          ...
          if (_sectionTab == 0) ...[
              ...
              SizedBox(
                ...
                child: ListView.separated(
                  ...
                  itemBuilder: (context, index) =>
                      _buildTargetGoalCard(lockedItems[index], availablePoints),
                ),
              ),
            ...
            if (filteredUnlocked.isEmpty)
              _buildEmptyStateWidget(
                ...
                subtitle:
                    'جرّب اختيار تصنيف آخر أو عد لاحقاً لرؤية العروض الجديدة',
                actionLabel: _selectedCategory == 'الكل'
                    ? '🔄 تحديث القائمة'
                    : '🌐 استكشاف كل التصنيفات',
                onAction: () {
                  if (_selectedCategory != 'الكل') {
              ...
              GridView.builder(
                ...
                itemBuilder: (context, index) =>
                    _buildAvailableRewardGridCard(filteredUnlocked[index]),
              ),
                  ...
                  Flexible(
                    child: ListView.separated(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        final name = (store['business_name'] ?? 'متجر')
                            .toString();
                        ...
                        return Container(
                          ...
                          child: Row(
                            children: [
                              CircleAvatar(
                                backgroundColor: Colors.brown.withValues(
                                  alpha: 0.15,
                                ),
                                child: const Icon(
                  ...
                  Flexible(
                    child: ListView.separated(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        final name = (item['coalition_name'] ?? 'ائتلاف')
                            .toString();
  ...
  Future<void> _filterByMerchant(String merchantId, String storeName) async {
    ...
    try {
      final fetched = await CompanyServerService.getRewards(
  ...
  Future<void> _filterByCoalition(
    String coalitionId,
