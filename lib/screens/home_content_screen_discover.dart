  ...
  return FutureBuilder<Map<String, dynamic>>(
    ...
    builder: (context, sourcesSnapshot) {
      ...
      for (final s in storeSourcesList) {
        final mId = (s['merchantId'] ?? s['storeId'] ?? s['id'] ?? '')
            .toString();
      ...
      return Column(
        children: stores
            ...
            .map((store) {
              ...
              final merchantId = (store['merchantId'] ?? store['id'] ?? '')
                  .toString();
              final userStorePoints =
                  storePointsMap[merchantId] ??
                  _toInt(store['userPoints'] ?? store['myPoints']);
              final cashbackRate =
                  (store['pointValue'] ?? store['cashbackRate'] ?? 5)
                      .toString();
              final imageUrl =
                  (store['imageUrl'] ??
                          store['coverUrl'] ??
              ...
              return Card(
                ...
                child: InkWell(
                  borderRadius: BorderRadius.circular(16),
                  onTap: () => Navigator.of(state.context).push(
                    MaterialPageRoute(
