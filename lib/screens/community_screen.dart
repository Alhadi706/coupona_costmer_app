          ...
          child: Container(
            ...
            child: TabBar(
              ...
              tabs: [
                ...
                Tab(text: 'community_marketplace_tab'.tr(), icon: const Icon(Icons.storefront)),
...
class _MarketplaceTabState extends State<_MarketplaceTab> {
  ...
  Future<void> _loadOffers() async {
    ...
    try {
      final res = await CompanyServerService.getCustomerCommunityOffers(
  ...
  Future<void> _contactSeller(Map<String, dynamic> offer) async {
    try {
      final offerId = (offer['id'] ?? '').toString();
      final res = await CompanyServerService.contactCommunityOfferSeller(offerId: offerId);
      if (!mounted) return;
      final chatId = (res['chatId'] ?? '').toString();
      final title = (res['offerTitle'] ?? offer['title'] ?? '').toString();
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('open_private_chat_error'.tr(namedArgs: {'error': e.toString()}))),
      );
    }
  }
  ...
  Future<void> _updateStatus(String offerId, String newStatus) async {
    try {
      await CompanyServerService.updateCommunityOfferStatus(offerId: offerId, status: newStatus);
      _loadOffers();
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
    ...
    await showDialog(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setDialogState) {
          ...
          final estimatedPoints = (priceVal * 10).round();

          return AlertDialog(
            title: Text('community_create_offer'.tr()),
            content: SingleChildScrollView(
              child: Column(
                mainAxisSize: MainAxisSize.min,
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  TextField(
                    ...
                    decoration: InputDecoration(
                      labelText: 'community_offer_title'.tr(),
                      border: const OutlineInputBorder(),
                    ),
                  ),
                  const SizedBox(height: 10),
                  TextField(
                    ...
                    maxLines: 3,
                    decoration: InputDecoration(
                      labelText: 'community_offer_desc'.tr(),
                      border: const OutlineInputBorder(),
                    ),
                  ),
                  const SizedBox(height: 10),
                  DropdownButtonFormField<String>(
                    value: category,
                    decoration: InputDecoration(
                      labelText: 'community_offer_category'.tr(),
                      border: const OutlineInputBorder(),
                    ),
                    items: [
                      DropdownMenuItem(value: 'FOOD', child: Text('community_cat_food'.tr())),
                      DropdownMenuItem(value: 'REAL_ESTATE', child: Text('community_cat_real_estate'.tr())),
                      DropdownMenuItem(value: 'SERVICES', child: Text('community_cat_services'.tr())),
                      DropdownMenuItem(value: 'RENTALS', child: Text('community_cat_rentals'.tr())),
                    ],
                    onChanged: (val) {
                  ...
                  TextField(
                    ...
                    decoration: InputDecoration(
                      labelText: 'community_offer_price'.tr(),
                      border: const OutlineInputBorder(),
                    ),
                  ...
                  const SizedBox(height: 10),
                  CheckboxListTile(
                    title: Text('community_accepts_points'.tr()),
                    subtitle: Text('community_points_valuation_note'.tr() +
                        (acceptsPoints ? '\n(تساوي $estimatedPoints نقطة)' : '')),
                    value: acceptsPoints,
                    onChanged: (val) => setDialogState(() => acceptsPoints = val ?? false),
                    contentPadding: EdgeInsets.zero,
                  ),
                  const SizedBox(height: 10),
                  const Text('نطاق الظهور:', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13)),
                  RadioListTile<String>(
                    title: Text('community_scope_public'.tr()),
                    subtitle: const Text('يظهر لجميع مستخدمي المنصة'),
                    value: 'PUBLIC_COMMUNITY',
                    groupValue: visibilityScope,
                    onChanged: (val) {
                      if (val != null) setDialogState(() => visibilityScope = val);
                    },
                    contentPadding: EdgeInsets.zero,
                  ),
                  RadioListTile<String>(
                    title: Text('community_scope_my_merchants'.tr()),
                    subtitle: const Text('يظهر فقط للتجار الذين تتعامل معهم'),
                    value: 'MY_MERCHANTS_ONLY',
                    groupValue: visibilityScope,
                    onChanged: (val) {
                      if (val != null) setDialogState(() => visibilityScope = val);
                    },
            ...
            actions: [
              ...
              ElevatedButton(
                onPressed: () async {
                  ...
                  try {
                    await CompanyServerService.createCustomerCommunityOffer(
                      ...
                      category: category,
                      priceLyd: double.tryParse(priceCtrl.text) ?? 0.0,
                      acceptsPointsTrade: acceptsPoints,
                ...
                child: Text('community_create_offer'.tr()),
              ),
            ],
          );
        },
                ...
                Row(
                  children: [
                    ...
                    ElevatedButton.icon(
                      ...
                      label: Text('community_create_offer'.tr()),
                    ),
                  ],
                ),
                const SizedBox(height: 8),
                SingleChildScrollView(
                  ...
                  child: Row(
                    children: [
                      ...
                      FilterChip(
                        label: Text('community_cat_food'.tr()),
                      ...
                      const SizedBox(width: 4),
                      FilterChip(
                        label: Text('community_cat_real_estate'.tr()),
                        selected: _selectedCategory == 'REAL_ESTATE',
                        onSelected: (_) {
                          setState(() => _selectedCategory = 'REAL_ESTATE');
                          _loadOffers();
                        },
                      ),
                      const SizedBox(width: 4),
                      FilterChip(
                        label: Text('community_cat_services'.tr()),
                      ...
                      const SizedBox(width: 4),
                      FilterChip(
                        label: Text('community_cat_rentals'.tr()),
                        selected: _selectedCategory == 'RENTALS',
                        onSelected: (_) {
                          setState(() => _selectedCategory = 'RENTALS');
                          _loadOffers();
                        },
                      ),
                      const SizedBox(width: 8),
                      ChoiceChip(
                        label: const Text('عروضي'),
                        selected: _myOnly,
                        onSelected: (val) {
                          setState(() => _myOnly = val);
                          _loadOffers();
                        },
                      ),
                    ],
          ...
          Expanded(
            child: _loading
                ...
                : _offers.isEmpty
                    ? Center(
                        child: Text(
                          'لا توجد عروض متاحة حالياً في سوق المجتمع',
                    ...
                    : RefreshIndicator(
                        ...
                        child: ListView.builder(
                          ...
                          itemBuilder: (ctx, i) {
                            ...
                            final price = (item['price_lyd'] ?? 0).toString();
                            final acceptsPoints = item['accepts_points_trade'] == true;
                            final pointsReq = item['points_required'] ?? 0;
                            final sellerName = (item['seller_name'] ?? 'عضو مجتمعي').toString();
                                      ...
                                      children: [
                                        if (acceptsPoints)
                                          Chip(
                                            ...
                                            label: Text(
                                              'community_points_trade_badge'.tr(namedArgs: {'points': '$pointsReq'}),
                                        ...
                                        Chip(
                                          ...
                                          label: Text(sellerName, style: const TextStyle(fontSize: 11)),
                                      ...
                                      children: [
                                        if (!isOwner)
                                          ElevatedButton.icon(
                                            ...
                                            label: Text('community_contact_seller'.tr()),
                                          )
                                        else
                                          DropdownButton<String>(
                                            value: status,
                                            items: const [
                                              DropdownMenuItem(value: 'ACTIVE', child: Text('متاح (نشط)')),
                                              DropdownMenuItem(value: 'SOLD', child: Text('تم البيع')),
                                              DropdownMenuItem(value: 'ARCHIVED', child: Text('مؤرشف')),
                                            ],
                                            onChanged: (newSt) {
                                              if (newSt != null) {
                                                _updateStatus((item['id'] ?? '').toString(), newSt);
                                              }
                                            },
                                          ),
                                      ],
