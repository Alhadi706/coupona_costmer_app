...
enum CommunityGroupSort { mostMembers, alphabetic }

class CommunityGroupsTab extends StatefulWidget {
  final String? initialGroupId;

  const CommunityGroupsTab({super.key, this.initialGroupId});

  @override
  State<CommunityGroupsTab> createState() => _CommunityGroupsTabState();
}

class _CommunityGroupsTabState extends State<CommunityGroupsTab> {
  final TextEditingController _searchController = TextEditingController();
  CommunityGroupSort _sort = CommunityGroupSort.mostMembers;
    ...
    return StreamBuilder<List<Map<String, dynamic>>>(
      ...
      builder: (context, snapshot) {
        ...
        filteredGroups.sort((a, b) {
          if (_sort == CommunityGroupSort.alphabetic) {
            ...
            Wrap(
              ...
              children: [
                ChoiceChip(
                  ...
                  selected: _sort == CommunityGroupSort.mostMembers,
                  onSelected: (_) => setState(() => _sort = CommunityGroupSort.mostMembers),
                ),
                ChoiceChip(
                  label: Text('alphabetical_order'.tr()),
                  selected: _sort == CommunityGroupSort.alphabetic,
                  onSelected: (_) => setState(() => _sort = CommunityGroupSort.alphabetic),
                ),
              ],
            ),
            const SizedBox(height: 12),
            ...
            child: Column(
              ...
              children: [
                ...
                ...options.asMap().entries.map((entry) => TextField(
                      decoration: InputDecoration(
                        labelText: 'community_poll_option'.tr(namedArgs: {'number': '${entry.key + 1}'}),
                      ),
...
class CommunityPrivateChatsTab extends StatefulWidget {
  const CommunityPrivateChatsTab({super.key});

  @override
  State<CommunityPrivateChatsTab> createState() => _CommunityPrivateChatsTabState();
}

class _CommunityPrivateChatsTabState extends State<CommunityPrivateChatsTab> {
